    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        for sample_idx in range(group_size):
            ...
            if args.model == 'OneShot':
                pred_molecule_list, true_molecule_list, products_list, scores, nlls, ells, pred_classes = model.sample_batch(
                    data=data,
                    batch_id=ident,
                    batch_size=bs,
                    save_final=0,
        ...
        for mol_idx_in_batch in range(bs):
            ...
            nlls_group = []
            ells_group = []
            classes_group = [] # 这里是初始化

            # zip 中的最后一个变量名应为 class_gr
            for batch_group, scores_group, nll_gr, ell_gr, class_gr in zip(batch_groups, batch_scores, batch_nll, batch_ell, batch_classes):
                ...
                classes_group.append(class_gr[mol_idx_in_batch]) # 这里使用 class_gr
            ...
            grouped_classes.append(classes_group) # 这里追加的是 classes_group

        # Writing smiles
        for true_mol, product_mol, pred_mols, pred_scores, nlls, ells, pred_cls_group in zip(
                ground_truth, input_products, grouped_samples, grouped_scores, grouped_nlls, grouped_ells, grouped_classes,
        ):
            ...
            # 内部循环，zip 中最后一个变量是 pred_cls_group
            for pred_mol, pred_score, nll, ell, pred_c in zip(pred_mols, pred_scores, nlls, ells, pred_cls_group):
