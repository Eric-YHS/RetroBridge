...
class RetroBridgeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, reaction_smiles in enumerate(tqdm(table['reactants>reagents>production'].values)):
            ...
            reaction_class = table['class'].values[i]
            num_reaction_classes = 10

            y = F.one_hot(torch.tensor([reaction_class - 1]), num_classes=num_reaction_classes).float()

            data = Data(
                x=r_x, edge_index=r_edge_index, edge_attr=r_edge_attr, y=y, idx=i, # y 已被修改
...
class RetroBridgeDatasetInfos:
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features, use_context):
        ...
        r_ex_dense, r_node_mask = utils.to_dense(
            ...
            example_batch.batch,
            y=example_batch.y # 传入 y
        )

        # 同样，为产物创建一个0维的y
        bs = example_batch.y.size(0)
        device = example_batch.y.device
        p_y = torch.zeros((bs, 0), device=device)
        ...
        self.output_dims = {
            'X': example_batch['x'].size(1),
            'E': example_batch['edge_attr'].size(1),
            'y': example_batch['y'].size(1)  # 从 0 修改为 y 的维度
        }

        # --- 打印维度信息 ---
        print("\n\n####################################################")
        print("###### Computing Model Input/Output Dimensions #####")
        print("####################################################\n")
        # --- 结束打印 ---
        
        print('Final Input dims computed:')
        for k, v in self.input_dims.items():
            print(f'\t{k} -> {v}')

        print('\nFinal Output dims computed:')
